...
class light_net_LSTM(nn.Module):
    def __init__(self, num_fault=9, device=None):
        super(light_net_LSTM, self).__init__()
        feat_dim = 4
        hid_dim = 128
        num_layers = 3
        ...
        self.linear_regress = nn.Linear(hid_dim * 2, feat_dim)

        self.linear_sig1 = nn.Linear(hid_dim*2*50, self.fault_type)
        self.linear_sig2 = nn.Linear(hid_dim*2*50, self.fault_type)
        self.linear_sig3 = nn.Linear(hid_dim*2*50, self.fault_type)
        self.linear_sig4 = nn.Linear(hid_dim*2*50, self.fault_type)
...
if __name__ == "__main__":
    ...
    reg, cls = net.forward(inputs, targ)
